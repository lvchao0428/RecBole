#!/usr/bin/env bash
# Two-phase TF-IDF + single-view LLM V3 + stratified — Food
...
python scripts/two_phase_train.py \
  --model SASRecAlignV3 \
  --dataset Food \
  --config_files "sasrec_align_food_qwen3_stratified_v3.yaml" \
  ...
  --checkpoint_dir "./saved/two_phase_run_tfidf_llm_v3_food_stratified_${CHECKPOINT_TAG}_seed${SEED}" \
  --seed "$SEED" \
  --variant_features "sasrec,tfidf,llm,v3,food,stratified,${CHECKPOINT_TAG},seed_${SEED}" \
  --watchdog_disable \
  --save
