#!/usr/bin/env bash
# Two-phase TF-IDF+LLM V3 + stratified — Grocery
...
python scripts/two_phase_train.py \
  --model SASRecAlignV3 \
  --dataset "$DS" \
  --config_files "sasrec_align_grocery_qwen_stratified_v3.yaml" \
  ...
  --checkpoint_dir "./saved/two_phase_run_tfidf_llm_v3_grocery_stratified_${CHECKPOINT_TAG}_seed${SEED}" \
  --seed "$SEED" \
  --variant_features "sasrec,tfidf,llm,v3,grocery,stratified,${CHECKPOINT_TAG},seed_${SEED}" \
  --watchdog_disable \
  --save
