...
# Two-phase strong baseline: SASRec_Align + TF-IDF (base only) on Amazon_Toys_and_Games
...
python scripts/two_phase_train.py \
  ...
  --config_files "sasrec_align_toys_base.yaml" \
  --phase_a_grid \
  ...
  --seed 2025 \
  --variant_features "sasrec,tfidf,toys" \
  --watchdog_interval 20 \
  --watchdog_log "run_metrics/watchdog_tfidf_toys.log" \
  --watchdog_cpu_gb 40 \
  --watchdog_gpu_gb 28 \
  --save
