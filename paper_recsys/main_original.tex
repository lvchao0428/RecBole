...
%
We propose \model, which introduces three coordinated design choices on top of a \base backbone: \emph{per-view whitening} that decorrelates multi-view \llm embeddings before fusion, \emph{explicit ID$\times$Text cross interactions} via DCN-V2, and \emph{multi-view contrastive alignment} with cold-start reweighting.
Through controlled ablations under full-ranking evaluation on Amazon Beauty and Toys\&Games, we empirically characterize an \textbf{HR--ranking trade-off} under full ranking: enabling the cross network improves ranking precision (MRR/NDCG) at the cost of recall (HR), while disabling it reverses this balance---a finding that directly maps to deployment choices between candidate generation and final re-ranking.
%
...
\keywords{Sequential Recommendation, Text-Enhanced Recommendation, \mbox{Multi-View} Representation, Feature Crossing, Contrastive Alignment, \mbox{Cold-Start}}
...
in-batch items. \mbox{Cold-start} reweighting prevents frequent items
from dominating the align\-ment signal, ensuring that
...
identity\slashbreak{}function\slashbreak{}audience\slashbreak{}category views), where $d_t$ and $d_l$ denote the
...
and fixed during training\slashbreak{}inference. Each view uses its own whitening
...
boosting), improving cold-start recommendations under the balanced
setting while keeping frequent-item performance competitive.
...
Single-view encodes the item title (``\texttt{[TITLE] \{title\}}'') via
mean-pooled Qwen2.5-7B hidden states into a 256-d embedding.
Multi-view uses four perspective prompts (identity, function, audience, category),
each independently mean-pooled, reduced to $d_v{=}64$ via SVD, and refined by
per-view SE-style gating before fusion.
