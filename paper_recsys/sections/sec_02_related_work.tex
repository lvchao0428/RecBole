...
Feature-augmented sequential models such as
FDSA~\cite{zhang2019fdsa} encode side features with a dual
Transformer, while S3-Rec~\cite{zhou2020s3rec} pretrains
sequence representations via self-supervised mutual information
objectives. We use GRU4Rec~\cite{hidasi2016gru4rec} and these
models as cross-architecture baselines under the same
...
classical term weighting~\cite{salton1988termweighting} remains a
strong and interpretable baseline in our full-ranking study.
...
BERT-encoded item text with parametric whitening and a
mixture-of-experts adaptor~\cite{shazeer2017outrageously} to
produce universal transferable representations, while
VQ-Rec~\cite{hou2023vqrec} bridges
...
session-based settings~\cite{chen2023kmvg} using
...
popularity strata~\cite{abdollahpouri2019unfairness}, and we
interpret HR--MRR/NDCG trade-offs through the lens of
candidate coverage versus top-position calibration~\cite{steck2018calibrated}.
