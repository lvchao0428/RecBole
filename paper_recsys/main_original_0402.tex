...
Through controlled ablations with four tunable knobs, we expose an HR--ranking trade-off governed by the cross network and show that multi-view prompting, under equal embedding bandwidth, improves ranking quality on cold-start items over all text baselines.
...
the training data. We report only full-ranking metrics in the
...
\texttt{MRR@10} for both Beauty and Toys\&Games.
...
cross network, contrastive alignment, and whitening on Toys\&Games (7B).
...
\midrule
\multicolumn{5}{l}{\textit{Contrastive alignment (RQ4)}} \\
...
Table~\ref{tab:ablation} (SE/cross/align/whitening). The dashed arrow
