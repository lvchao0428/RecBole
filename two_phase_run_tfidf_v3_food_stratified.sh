#!/usr/bin/env bash
# Two-phase TF-IDF V3 + stratified — Food
...
python scripts/two_phase_train.py \
  --model SASRecAlignV3 \
  --dataset Food \
  --config_files "sasrec_align_food_base_stratified_v3.yaml" \
  ...
  --checkpoint_dir "./saved/two_phase_run_tfidf_v3_food_stratified_${CHECKPOINT_TAG}_seed${SEED}" \
  --seed "$SEED" \
  --variant_features "sasrec,tfidf,v3,food,stratified,${CHECKPOINT_TAG},seed_${SEED}" \
  --watchdog_disable \
  --save
