# -*- coding: utf-8 -*-
"""
UniSRecAlignV3 - UniSRec backbone with V3 text pipeline (cross + align)

Design:
- Inherits SASRecAlignV3 for cross/align/gate logic
...
- V3 scaffold (DCN-V2 cross, InfoNCE align, cold boost) works on top

Three experiment rows:
1. UniSRec Base    -> original unisrec.py (ID + MoE, no cross/align)
2. UniSRecAlignV3  -> this file (ID + MoE + cross + align)
3. UniSRecAlignMultiViewV3 -> unisrec_align_multiview_v3.py (+ multi-view)
"""
...
class UniSRecAlignV3(SASRecAlignV3):
    """
    UniSRec + V3 text pipeline.

    Differences from SASRecAlignV3:
    - Loads a PLM embedding table and adapts it via MoE adaptor
    - forward() adds MoE(text) to item_embedding before Transformer
      (transductive mode, same as original UniSRec)
    - V3 cross/align/gate still operates on item-level embeddings for scoring
    """

    def __init__(self, config, dataset):
        # UniSRec-specific params (read before super().__init__)
        ...
        if plm_emb is None:
            raise ValueError(
                "UniSRecAlignV3 requires PLM embeddings. "
        ...
        # MoE Adaptor: plm_dim -> hidden_size
        ...
        self.logger.info(
            "UniSRecAlignV3: MoE adaptor (%d experts, plm_dim=%d -> hidden=%d)",
            self._unisrec_n_exps, plm_dim, self.hidden_size,
        )

    def _load_plm_embeddings(self):
        """Load PLM embeddings for MoE adaptor (separate from V3 text pipeline)."""
