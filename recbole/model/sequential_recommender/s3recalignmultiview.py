"""
S3RecAlignMultiView - Multi-View Text Features for S3Rec
Based on SASRecAlignMultiViewV2 architecture

NOTE: S3Rec has two stages: pretrain and finetune.
Text alignment and multi-view features are only applied during the finetune stage.
"""
...
from recbole.model.sequential_recommender.s3rec_align import S3RecAlign, DCNV2Cross
from recbole.model.layers import MLPLayers


class S3RecAlignMultiView(S3RecAlign):
    """
    S3RecAlign variant with per-view multi-view embeddings and alignment.
    ...
    def __init__(self, config, dataset):
        ...
        # Only initialize multiview modules for finetune stage
        if self.use_text_view_split and self.train_stage == "finetune":
            ...
            # n_items includes mask token, text embeddings exclude mask token
            ...
            for view in prompts:
                ...
                # Verify size matches
                if npy.shape[0] != actual_n_items:
                    self.logger.warning(
                        f"S3RecAlignMultiView: view {idx} has {npy.shape[0]} items but expected {actual_n_items}"
                    )
            ...
            self.logger.info(
                "S3RecAlignMultiView initialized: %d views, SENet=%s (ratio=%d), per-view L2 norm=%s",
                self.num_text_views, 
    ...
    def _gather_text_views(self, ids_flat: torch.Tensor) -> torch.Tensor:
        ...
        """
        if not self.use_text_view_split or self.train_stage == "pretrain":
            raise RuntimeError("text view split not enabled or in pretrain stage.")
        ...
        for idx, proj in enumerate(self.text_view_proj):
            ...
            if view_emb_table.device != ids_clamped.device:
                view_emb_table = view_emb_table.to(ids_clamped.device)
            gathered = view_emb_table[ids_clamped]
    ...
    def _get_fused_item_embeddings(self, item_ids: torch.Tensor = None) -> torch.Tensor:
        ...
        if self.item_text_emb_base is not None:
            ids_clamped = torch.clamp(all_ids, 0, self.n_items - 2)
            base_feat = self.item_text_emb_base[ids_clamped]
            if self.detach_text_emb:
    ...
    def _fuse_with_cross_network(
        ...
        ids_clamped = torch.clamp(item_ids, 0, self.item_popularity.size(0) - 1)
    ...
    def calculate_loss(self, interaction):
        ...
        """
        # For pretrain stage, use parent's pretrain loss
        ...
        if (
            ...
            cold_start_weights = self._compute_cold_start_weights(pos_items)
            ...
            if not getattr(self, '_multiview_align_debug_logged', False):
                try:
                    self.logger.info(
                        "S3RecAlignMultiView: per-view alignment enabled | "
...
class S3RecAlignMultiViewV2(S3RecAlignMultiView):
    """
    Enhanced version of S3RecAlignMultiView with same features as SASRecAlignMultiViewV2.
    """
    pass


# Aliases
S3Rec_Align_MultiView = S3RecAlignMultiView
S3Rec_Align_MultiView_V2 = S3RecAlignMultiViewV2

__all__ = [
    "S3RecAlignMultiView", 
    "S3RecAlignMultiViewV2",
    "S3Rec_Align_MultiView",
    "S3Rec_Align_MultiView_V2"
]
