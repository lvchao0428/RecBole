# -*- coding: utf-8 -*-
"""
GRU4RecAlign V3 - GRU backbone with V3 text pipeline

GRU4Rec backbone with the V3 text pipeline (DCN-V2 cross, InfoNCE alignment, cold-start reweighting).
...
class GRU4RecAlignV3(SequentialRecommender):
    """
    GRU4RecAlign V3 - GRU backbone with V3 text pipeline
    ...
    def __init__(self, config, dataset):
        super(GRU4RecAlignV3, self).__init__(config, dataset)

        # load GRU backbone parameters
        ...
        # Explicit switch to fully disable text features and mimic pure GRU4Rec
        ...
        if self.disable_text_feature:
            ...
            if self.use_llm:
                if self.item_text_emb_llm is None:
                    raise ValueError(
                        "GRU4RecAlignV3: use_llm=True but item_text_emb_path_llm is missing."
                    )
            else:
                if self.item_text_emb_base is None:
                    raise ValueError(
                        "GRU4RecAlignV3: text features are enabled but item_text_emb_path_base is missing."
                    )
        ...
        if text_in_dim > 0:
            if self.fused_item_norm_flag:
                self.item_emb_norm = nn.LayerNorm(self.embedding_size, eps=self.layer_norm_eps)

            if self.use_cross:
                self.text_cross = DCNV2Cross(text_in_dim, num_layers=self.text_cross_layer_num)
                self.text_deep = MLPLayers([text_in_dim, self.embedding_size], dropout=0.0, bn=self.text_mlp_bn)
                self.text_predictor = nn.Linear(text_in_dim + self.embedding_size, self.embedding_size)

                fusion_input_dim = self.embedding_size + text_in_dim
                self.item_fusion_cross = DCNV2Cross(fusion_input_dim, num_layers=self.text_cross_layer_num)
                self.item_fusion_deep = MLPLayers([fusion_input_dim, self.embedding_size], dropout=0.0, bn=self.text_mlp_bn)
                self.item_fusion_predictor = nn.Linear(fusion_input_dim + self.embedding_size, self.embedding_size)
                ...
                self.item_text_proj = nn.Linear(text_in_dim, self.embedding_size)
                self.item_concat_predictor = nn.Linear(self.embedding_size * 2, self.embedding_size)

            if self.text_proj_norm_flag:
                self.text_proj_norm = nn.LayerNorm(self.embedding_size, eps=self.layer_norm_eps)

            if self.fused_item_norm_flag:
                self.fused_item_norm = nn.LayerNorm(self.embedding_size, eps=self.layer_norm_eps)
        ...
        if self.align_weight > 0.0 and self._has_item_text():
            self.logger.info(
                "GRU4RecAlignV3: text_mode=%s, align_weight=%.3f, cold_text_boost=%.2f, infer_boost=%.2f, cold_threshold=%d",
    ...
    def set_freeze(self, freeze: bool) -> None:
        ...
        for p in self.gru_layers.parameters():
            p.requires_grad_(not freeze)
        for p in self.dense.parameters():
            p.requires_grad_(not freeze)
        for p in self.emb_dropout.parameters():
            p.requires_grad_(not freeze)
    ...
    def _init_weights(self, module):
        if isinstance(module, (nn.Linear, nn.Embedding)):
            ...
            for layer_idx in range(module.num_layers):
                xavier_uniform_(getattr(module, f"weight_hh_l{layer_idx}"))
                xavier_uniform_(getattr(module, f"weight_ih_l{layer_idx}"))
    ...
    def _project_text(self, raw: torch.Tensor) -> torch.Tensor:
        if raw.size(1) == 0:
            return torch.zeros((raw.size(0), self.embedding_size), device=raw.device)
        if self.use_cross and self.text_cross is not None:
            ...
            proj = torch.zeros((raw.size(0), self.embedding_size), device=raw.device)
    ...
    def calculate_loss(self, interaction):
        ...
        if (
            ...
            if not self._align_debug_logged:
                try:
                    self.logger.info(
                        "GRU4RecAlignV3: first-step align_loss=%.6f, batch_pos=%d",
        ...
        if (not self._gate_debug_logged) and self.training:
            try:
                ...
                self.logger.info(
                    "GRU4RecAlignV3: text_gate_alpha=%.4f, text_mode=%s, infer_boost=%.2f",
...
# Alias
GRU4Rec_Align_V3 = GRU4RecAlignV3
