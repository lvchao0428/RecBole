"""
SASRecAlignMultiViewV3 - Simplified Multi-View Text Features

简化权重配置，与 sasrec_align_v3.py 保持一致：
...
- 继承自 SASRecAlignV3 而非 SASRecAlign
...
class SASRecAlignMultiViewV3(SASRecAlignV3):
    """
    SASRecAlignMultiViewV3 - 简化版多视图文本特征模型
    
    继承自 SASRecAlignV3，复用其简化的权重配置：
    - align_weight: 全局对齐权重
    ...
    def __init__(self, config, dataset):
        ...
        if self.use_text_view_split:
            ...
            # 日志
            self.logger.info(
                "SASRecAlignMultiViewV3: %d views, SENet=%s (ratio=%d), per_view_l2_norm=%s",
                self.num_text_views,
        ...
        if (
            ...
            if not self._multiview_align_debug_logged:
                try:
                    ...
                    self.logger.info(
                        "SASRecAlignMultiViewV3: per-view align | total=%.6f | losses=[%s]",
