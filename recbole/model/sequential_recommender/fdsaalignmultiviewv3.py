"""
FDSAAlignMultiViewV3 - Simplified Multi-View Text Features

简化权重配置，与 fdsaalignv3.py 保持一致：
...
- 继承自 FDSAAlignV3 而非 SASRecAlign
...
from recbole.model.sequential_recommender.fdsaalignv3 import FDSAAlignV3, DCNV2Cross


class FDSAAlignMultiViewV3(FDSAAlignV3):
    """
    FDSAAlignMultiViewV3 - 简化版多视图文本特征模型

    继承自 FDSAAlignV3，复用其简化的权重配置：
    - align_weight: 全局对齐权重
    ...
    def __init__(self, config, dataset):
        ...
        if self.use_text_view_split:
            ...
            for view in prompts:
                ...
                # 投影层: view_dim → hidden_size
                self.text_view_proj.append(nn.Linear(view_dim, self.hidden_size))

                # SENet: hidden_size → reduction → hidden_size
            ...
            # 日志
            self.logger.info(
                "FDSAAlignMultiViewV3: %d views, SENet=%s (ratio=%d), per_view_l2_norm=%s",
                self.num_text_views,
        ...
        if (
            ...
            if not self._multiview_align_debug_logged:
                try:
                    ...
                    self.logger.info(
                        "FDSAAlignMultiViewV3: per-view align | total=%.6f | losses=[%s]",
...
# Aliases
FDSA_Align_MultiView_V3 = FDSAAlignMultiViewV3

__all__ = ["FDSAAlignMultiViewV3", "FDSA_Align_MultiView_V3"]
