...
python tools/build_item_text_emb_qwen3_hf.py \
  ...
  --output "dataset/Amazon_Beauty/item_text_emb.${MODEL_NAME}.base.npy" \
  --prompt_preset base \
  --output_mode mean \
  --project_dim 256 \
  --dataset Amazon_Beauty \
...
python tools/build_item_text_emb_qwen3_hf.py \
  --mapping "$MAPPING_FILE" \
  --model_name_or_path "$MODEL_PATH" \
  --output "dataset/Amazon_Beauty/item_text_emb.${MODEL_NAME}.multiview.npy" \
  --prompt_preset multiview \
  --output_mode concat \
  --split_output_dir "dataset/Amazon_Beauty/${MODEL_NAME}_4views" \
  --view_project_dim 64 \
  --dataset Amazon_Beauty \
  --config sasrec_base_plain.yaml recbole/properties/overall.yaml \
  --batch_size 16 \
...
echo "   - 视图3 (Category):  dataset/Amazon_Beauty/${MODEL_NAME}_4views/view_3.npy"
echo "   - 元数据: dataset/Amazon_Beauty/${MODEL_NAME}_4views/views.json"
echo "完成时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
echo ""
echo "  [${MODEL_NAME}单视图]"
echo "    - dataset/Amazon_Beauty/item_text_emb.${MODEL_NAME}.base.npy"
echo ""
echo "  [${MODEL_NAME}多视图]"
echo "    - dataset/Amazon_Beauty/item_text_emb.${MODEL_NAME}.multiview.npy"
