...
# ==============================================
# 使用 Qwen2.5-7B-Instruct 生成 Toys 文本特征
...
python tools/build_item_text_emb_base.py \
  ...
  --output dataset/${DATASET_NAME}/item_text_emb.base.no_whiten.npy \
  --title_field title \
  --svd_dim 256 \
  --svd_random_state 42 \
  --ngram_min 1 \
...
echo "   - 特征文件: dataset/${DATASET_NAME}/item_text_emb.base.no_whiten.npy"
...
python tools/build_item_text_emb_qwen3_hf.py \
  ...
  --output "dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.base.no_whiten.npy" \
  --prompt_preset base \
  --output_mode mean \
  --project_dim 256 \
  --dataset ${DATASET_NAME} \
...
echo "   - 特征文件: dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.base.no_whiten.npy"
echo "完成时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
python tools/build_item_text_emb_qwen3_hf.py \
  --mapping "$MAPPING_FILE" \
  --model_name_or_path "$MODEL_PATH" \
  --output "dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.multiview.no_whiten.npy" \
  --prompt_preset multiview \
  --output_mode concat \
  --split_output_dir "dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten" \
  --view_project_dim 64 \
  --dataset ${DATASET_NAME} \
  --config sasrec_base_plain.yaml recbole/properties/overall.yaml \
  --batch_size 32 \
...
echo "   - 拼接特征: dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.multiview.no_whiten.npy (shape: [N, 256])"
echo "   - 视图0 (Identity):  dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/view_0.npy"
echo "   - 视图1 (Function):  dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/view_1.npy"
echo "   - 视图2 (Audience):  dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/view_2.npy"
echo "   - 视图3 (Category):  dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/view_3.npy"
echo "   - 元数据: dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/views.json"
echo "完成时间: $(date '+%Y-%m-%d %H:%M:%S')"
echo ""
...
echo "生成的特征文件："
echo "  [TF-IDF基线]"
echo "    - dataset/${DATASET_NAME}/item_text_emb.base.no_whiten.npy"
echo ""
echo "  [${MODEL_NAME}单视图]"
echo "    - dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.base.no_whiten.npy"
echo ""
echo "  [${MODEL_NAME}多视图]"
echo "    - dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.multiview.no_whiten.npy"
echo "    - dataset/${DATASET_NAME}/${MODEL_NAME}_4views_no_whiten/ (分视图)"
echo ""
echo "下一步："
echo "  1. 验证特征维度:"
echo "     python -c \"import numpy as np; x=np.load('dataset/${DATASET_NAME}/item_text_emb.${MODEL_NAME}.base.no_whiten.npy'); print(f'shape={x.shape}, dtype={x.dtype}')\""
