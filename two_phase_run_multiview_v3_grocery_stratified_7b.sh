#!/usr/bin/env bash
# Two-phase Multi-View V3 (7B) + stratified — Grocery
...
python scripts/two_phase_train.py \
  ...
  --dataset "$DS" \
  --config_files "sasrec_align_multi_view_v3_grocery_stratified_7b.yaml" \
  ...
  --checkpoint_dir "./saved/two_phase_run_multiview_v3_grocery_stratified_7b_${CHECKPOINT_TAG}_seed${SEED}" \
  --seed "$SEED" \
  --variant_features "sasrec,multiview_v3,7b,grocery,stratified,${CHECKPOINT_TAG},seed_${SEED}" \
  --watchdog_disable \
  --save
