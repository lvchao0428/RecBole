#!/usr/bin/env bash
#
# exp_sensitivity_lambda_015.sh - Sensitivity Analysis: alignment_weight = 0.15
#
# 目的: 验证强对齐对 HR vs MRR trade-off 的影响
# 基准: λ=0.10, τ=0.05, cold=2.0, infer=1.0
# 变化: λ=0.10 → 0.15 (强对齐)
# 预期: HR↓ (覆盖受限), MRR↑ (排序质量提升)
#
...
GPU_ID=${GPU_ID:-0}
echo "========================================="
echo "Sensitivity Analysis: alignment_weight = 0.15"
echo "Using GPU: $GPU_ID"
echo "========================================="
echo "  - alignment_weight: 0.10 → 0.15 (强对齐)"
...
python scripts/two_phase_train.py \
  ...
  --checkpoint_dir ./saved/sensitivity_lambda_015 \
  --seed 2025 \
  --variant_features "sasrec,multiview_v2,7b,toys,lambda_015" \
  --watchdog_disable \
  --save

echo "✅ Done! Check: saved/sensitivity_lambda_015/"
