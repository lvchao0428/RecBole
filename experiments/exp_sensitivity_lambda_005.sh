#!/usr/bin/env bash
#
# exp_sensitivity_lambda_005.sh - Sensitivity Analysis: alignment_weight = 0.05
#
# 目的: 验证弱对齐对 HR vs MRR trade-off 的影响
# 基准: λ=0.10, τ=0.05, cold=2.0, infer=1.0
# 变化: λ=0.10 → 0.05 (弱对齐)
# 预期: HR↑ (覆盖更好), MRR↓ (排序质量下降)
#
...
GPU_ID=${GPU_ID:-0}
echo "========================================="
echo "Sensitivity Analysis: alignment_weight = 0.05"
echo "Using GPU: $GPU_ID"
echo "========================================="
echo "  - alignment_weight: 0.10 → 0.05 (弱对齐)"
...
python scripts/two_phase_train.py \
  ...
  --checkpoint_dir ./saved/sensitivity_lambda_005 \
  --seed 2025 \
  --variant_features "sasrec,multiview_v2,7b,toys,lambda_005" \
  --watchdog_disable \
  --save

echo "✅ Done! Check: saved/sensitivity_lambda_005/"
