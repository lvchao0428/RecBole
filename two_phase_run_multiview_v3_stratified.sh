#!/usr/bin/env bash
#set -euo pipefail

# Multi-View V3 Training with Stratified Evaluation
# 使用简化的 V3 权重配置
#
# V3 简化权重配置（与 SASRecAlignV3 统一）：
# ============================================================
# align_weight: 0.1       全局对齐权重
# cold_text_boost: 3.0    冷启动训练增强（训练时给低频商品更高对齐权重）
# infer_boost: 0.0        推理增强（推理时给低频商品更强文本信号）
# cold_threshold: 10      冷启动阈值
# ============================================================
#
# Multi-View 特有配置（保留）：
# - text_view_senet_ratio: 2 (SENet 压缩比)
# - per_view_l2_norm: true (每个视图独立 L2 归一化)
# - use_text_view_senet: true (启用 SENet 增强)

export PYTHONPATH="$(pwd):${PYTHONPATH:-}"
export PYTORCH_CUDA_ALLOC_CONF="expandable_segments:True"

# GPU ID support - can be set via environment variable or defaults to 0
GPU_ID=${GPU_ID:-0}
PHASE_A_EPOCHS="${PHASE_A_EPOCHS:-20}"
PHASE_B_EPOCHS="${PHASE_B_EPOCHS:-50}"
CHECKPOINT_TAG="${CHECKPOINT_TAG:-phaseb50}"
echo "Using GPU: $GPU_ID"

echo "========================================="
echo "Multi-View V3 with Stratified Metrics"
echo "========================================="
echo ""
echo "V3 Simplified Weight Config (unified with SASRecAlignV3):"
echo "  - align_weight: 0.1 (global alignment weight)"
echo "  - cold_text_boost: 3.0 (training cold-start boost)"
echo "  - infer_boost: 0.0 (inference cold-start boost)"
echo "  - cold_threshold: 10 (cold-start threshold)"
echo ""
echo "Multi-View Specific Config:"
echo "  - text_view_senet_ratio: 2"
echo "  - per_view_l2_norm: true"
echo "  - use_text_view_senet: true"
echo ""
echo "Item Stratification:"
echo "  - new:      [1, 3)   interactions"
echo "  - few:      [3, 10)  interactions"
echo "  - frequent: [10, +∞) interactions"
echo ""

python scripts/two_phase_train.py \
  --model SASRecAlignMultiViewV3 \
  --dataset Amazon_Beauty \
  --config_files "sasrec_align_multi_view_v3_stratified.yaml" \
  --config_dict "{'align_weight': 0.1, 'cold_text_boost': 3.0, 'infer_boost': 0.6, 'cold_threshold': 10}" \
  --gpu_id $GPU_ID \
  --phase_a_grid \
  --align_grid "0.10" \
  --tau_grid "0.05" \
  --backbone_burnin_epochs 0 \
  --burnin_eval_step 2 \
  --phase_a_epochs ${PHASE_A_EPOCHS} \
  --phase_a_eval_step 1 \
  --phase_a_valid_metric "MRR@10" \
  --metric_baseline 0.0272 \
  --metric_gain_threshold 0.01 \
  --lr_text_head 2e-3 \
  --lr_dnn_cross 5e-4 \
  --phase_a_auto_to_b \
  --phase_b_epochs ${PHASE_B_EPOCHS} \
  --backbone_lr_scale 0.1 \
  --checkpoint_dir ./saved/phase_runs_multiview_v3_stratified_${CHECKPOINT_TAG} \
  --seed 2025 \
  --variant_features "sasrec,multiview_v3,7b,4views,beauty,stratified,${CHECKPOINT_TAG}" \
  --watchdog_disable \
  --save

echo ""
echo "========================================="
echo "✅ Training Done!"
echo "========================================="
echo "V3 Model Features:"
echo "  - Simplified 3-weight config (align_weight, cold_text_boost, infer_boost)"
echo "  - Unified weight naming with SASRecAlignV3"
echo "  - Removed multiview_align_scale, IPW, gate regularization"
echo ""
echo "Multi-View Features:"
echo "  - 4-view Qwen embeddings with SENet enhancement"
echo "  - Per-view L2 normalization"
echo "  - Per-view learnable gate weights"
echo ""
echo "Stratified metrics in results:"
echo "  - Recall_new@10, Recall_few@10, Recall_frequent@10"
echo "  - NDCG_new@10, NDCG_few@10, NDCG_frequent@10"
echo "  - MRR_new@10, MRR_few@10, MRR_frequent@10"
echo ""
