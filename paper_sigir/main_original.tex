...
Through controlled ablations with four tunable knobs, we expose an HR--ranking trade-off governed by the cross network and show that multi-view prompting, under equal embedding bandwidth, most benefits cold-start items.
All code and configurations are available at \texttt{\url{https://anonymous.4open.science/r/recbole-multiview-D83A}}.
\end{abstract}
...
(see the anonymous repository for validated configurations).
...
and reliability; a sanity check comparing sampled vs.\ full
ranking is provided in the anonymous repository. Our evaluation
...
(complete hyperparameter settings including cold-start boosting
are provided in the anonymous repository); additional grids can
...
We did not observe stable, interpretable gains when scaling the
\llm encoder from 7B to 14B parameters; we therefore use 7B as
the default configuration (a small comparison table is provided
...
and MRR\_new@10 on both datasets, indicating stronger ranking
quality among cold-start items even when recall coverage is
slightly below the ID-only baseline on Beauty.


\begin{table*}[t]
\caption{Main results under full ranking (mean$\pm$std over 4 seeds). Best results per dataset in \textbf{bold}. $^{*}$~$p{<}0.05$, $^{**}$~$p{<}0.01$; paired $t$-test on per-user metric differences against the strongest baseline (Beauty: 146{,}995 test users; Toys: 164{,}590 test users). All values are percentages (\%). Extended strata results (few/frequent) are provided in the anonymous repository.}
\label{tab:main}
\centering
\small
...
\textbf{(2) Multi-view benefits cold-start items under equal bandwidth.}
Under equal embedding bandwidth ($4{\times}64$-d vs.\ $1{\times}256$-d),
multi-view \llm prompting consistently outperforms single-view,
with the most pronounced gains on the \textbf{new} stratum.
...
\smallskip\noindent
\textbf{Reproducibility.} All code, configurations, and auxiliary
