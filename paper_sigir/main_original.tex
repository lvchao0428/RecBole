...
\begin{table*}[t]
\caption{Main results under full ranking (mean$\pm$std over 4 seeds). Best results per dataset in \textbf{bold}. $^{*}$~$p{<}0.05$, $^{**}$~$p{<}0.01$, paired $t$-test against the strongest baseline on per-user metric differences. All values are percentages (\%). Extended strata results (few/frequent) are provided in the anonymous repository.}
\label{tab:main}
\centering
\small
