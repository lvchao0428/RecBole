...
%
We present \model, a fully open-sourced, plug-and-play framework that combines multi-view text representations (\tfidf and \llm, with per-view whitening for decorrelation), explicit ID$\times$Text cross interactions via DCN-V2, and contrastive alignment with cold-start reweighting.
The entire pipeline is released as a single-command reproducible package built on RecBole.
%
Under full-ranking evaluation on Amazon Beauty and Toys\&Games, \model achieves HR@10 of \textbf{5.79}\% and \textbf{6.61}\% (+6.8\% and +11.7\% over ID-only), while \tfidf alone already provides a strong baseline (+2.6\% and +6.3\% HR@10).
Through controlled ablations we expose an HR--ranking trade-off governed by the cross network and show that, under equal embedding bandwidth, multi-view prompting narrows the cold-start HR\_new gap to ID-only on Beauty (1.76\% vs.\ 1.88\%) while exceeding it on Toys (2.04\% vs.\ 1.92\%), and achieves the best cold-start ranking quality (NDCG\_new and MRR\_new) on both datasets.
All code and configuration files needed to reproduce the reported results are available at \texttt{\url{https://anonymous.4open.science/r/recbole-multiview-D83A}}.
\end{abstract}
...
In large-scale recommendation, teams typically budget cost along
three axes: \emph{trainable parameters} and optimizer state (memory),
\emph{forward-pass FLOPs} or measured latency (inference), and
\emph{offline preprocessing} for text (e.g., one-time encoder runs),
which is amortized over the catalog and separate from per-request
serving.
We report trainable parameters and framework-estimated FLOPs for our
Beauty configurations (Table~\ref{tab:params}) as comparable proxies
for runtime cost, and we discuss offline \llm view encoding relative
to single-view in \S\ref{sec:complexity}.
...
\begin{itemize}[leftmargin=*,itemsep=1pt]
  ...
  \item We benchmark on two Amazon domains (Beauty and
    ...
    popularity-stratified analysis (with the \textbf{new} stratum in the main tables), and provide systematic
    ablations (whitening, SE-style gating, cross) alongside robustness
    analyses (hyperparameter sensitivity/stability).
...
positives/negatives in training; note that negative sampling is
used only for training efficiency---all evaluation metrics are
computed under full ranking over the entire item set), and
...
\textbf{new} stratum, which is most informative for cold-start; we omit
\textbf{few} and \textbf{frequent} subtables for brevity.
...
$\{2024, 2025, 2026, 2027\}$ and report mean$\pm$std;
full-ranking metrics are evaluated over $K\in\{5,10,20\}$
...
FLOPs (TensorFlow trainer output) on Amazon Beauty ($|I|=12,102$,
...
\end{tabular}
\vspace{0.35em}
{\scriptsize Values rounded from training logs (exact counts:
\num{67161345}, \num{68082433}, \num{68803585}, \num{68937733} parameters;
FLOPs \num{39505256}, \num{39899496}, \num{39965032}, \num{40686440}).}
\end{table}
...
pronounced on the \textbf{new} stratum: on Beauty it narrows the
HR\_new gap to ID-only (1.76\% vs.\ 1.88\%), while on Toys it
exceeds it (2.04\% vs.\ 1.92\%). Diverse prompt perspectives
...
reproducible package on RecBole. Full-ranking experiments on
Amazon Beauty and Toys\&Games yield three actionable takeaways:
...
multi-view \llm prompting improves over text baselines and yields the
strongest ranking quality (NDCG/MRR) on the \textbf{new} stratum.
On Beauty it narrows the HR\_new gap to ID-only
(1.76\% vs.\ 1.88\%), while on Toys it exceeds it
(2.04\% vs.\ 1.92\%).
...
\smallskip\noindent
\textbf{Reproducibility.} All code and configuration files needed to
reproduce the experiments and figures reported in this paper are available at
