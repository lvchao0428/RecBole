...
Two practical observations motivate this work. First, while
\tfidf provides a strong baseline (e.g., +20.0\% HR@10 and
+38.7\% NDCG@10 on Beauty under full ranking), naively adding
single-view \llm embeddings yields only marginal further gains.
However, our multi-view approach with explicit cross and alignment
unlocks substantially larger improvements: \model achieves +29.4\%
HR@10 and +46.1\% NDCG@10 over ID-only on Beauty, demonstrating
that \textit{how} text features are integrated matters as much as
\textit{what} features are used. Second, model selection in industry
often relies on fast sampled evaluation (uniform-100 negatives), but
...
\end{equation}
This per-view normalization is controlled by the \texttt{per\_view\_l2\_norm}
configuration flag (enabled by default). Combined with whitening,
it addresses scale mismatch across views without conflating their
semantic content.
...
Text embeddings are generated offline using privately deployed
Qwen2.5 models (7B/14B/32B) on a single machine with 8$\times$RTX 4090.
Embedding extraction takes approximately 4 hours for 7B, 6 hours for 14B,
and 10 hours for 32B on the Beauty dataset (${\sim}$12K items).
Model training is conducted on a single RTX 5090; Phase-A takes
${\sim}$50 minutes for all configurations, while Phase-B varies by
model complexity: ${\sim}$3 hours for ID-only, ${\sim}$4 hours for
\tfidf, ${\sim}$5 hours for \tfidf+\llm, and ${\sim}$7 hours for
multi-view.
...
We compare the following configurations to isolate the contribution
of each component:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{\base (ID-only).} Pure sequential baseline
    without text features, serving as the collaborative-only
    reference.
  \item \textbf{\base + \tfidf.} A lightweight text baseline
    using precomputed \tfidf embeddings (256-d) with explicit
    cross and alignment, demonstrating the gains from
    inexpensive text features.
  \item \textbf{\base + \tfidf + \llm (single-view).} Adds a
    single \llm embedding (Qwen2.5-7B, 256-d) concatenated with
    \tfidf, using SE-style gating and alignment, representing
    the conventional single-view approach.
  \item \textbf{\model (multi-view).} Our proposed framework:
    \tfidf base view + 4 multi-view \llm prompt embeddings
    (identity/function/audience/category, each 64-d, total 256-d)
    with per-view SE-style blocks, explicit cross, and multi-view
    contrastive alignment.
\end{itemize}
All text-enhanced configurations share the same training protocol
(two-phase) and hyperparameter search space for fair comparison.
...
We grid-search alignment weight $\lambda\in\{0.05, 0.10, 0.15\}$,
temperature $\tau\in\{0.03, 0.05, 0.07\}$, cold-start boost
$\in\{1.5, 2.0, 2.5, 3.0\}$, and inference boost $\in\{0.5, 1.0, 1.5, 2.0, 2.5\}$.
The final configurations are $\lambda{=}0.10$, $\tau{=}0.05$ on both datasets,
with cold\_boost${=}2.5$ and infer\_boost${=}1.5$ for the Aggressive setting
(see Appendix~\ref{app:hyper} for sensitivity analysis).
...
\begin{table}[t]
\caption{Ablation: SE-style gating and Cross Network on multi-view 7B (Toys\&Games). Relative changes derived from cross-dataset validation. All values are percentages (\%).}
...
most metrics). The combination of removing both SE-style gating and Cross
shows the largest HR\_new@10 gains (+15.8\%), indicating that these
components together calibrate toward frequent items at the expense of
cold-start recall.

\paragraph{Cold-start analysis.}
Removing cross improves HR\_new@10 (+14.8\%), confirming that the cross
module emphasizes frequent items. This implies a design choice:
practitioners prioritizing cold-start HR should consider lighter fusion,
while those prioritizing top-1 accuracy (MRR) should retain cross.
...
\paragraph{Results on Beauty.}
Multi-view shows modest improvements over single-view under the same
embedding budget, with slightly better ranking metrics across all
configurations.

\paragraph{Results on Toys.}
Multi-view demonstrates more pronounced gains, particularly on the
\textbf{few} stratum where cold-start items benefit from diverse
semantic perspectives captured by multiple prompts.

\paragraph{Observation.}
The effectiveness of multi-view varies across domains: Toys exhibits
larger gains, likely due to richer product semantics (e.g., toy
functionality, age groups) that naturally align with our prompt
design (identity/function/audience/category).

\subsection{Summary of Ablation Findings}

\begin{table}[t]
\caption{Summary: component contributions to model performance based on ablation experiments (Toys 7B).}
\label{tab:ablation_summary}
\centering
\small
\begin{tabular}{lcc}
\toprule
\textbf{Component} & \textbf{HR@10} & \textbf{MRR/NDCG} \\
\midrule
Cross Network & $\downarrow$ ($-$8.4\%) & $\uparrow\uparrow$ (+12.7\%/+4.7\%) \\
SE-style gating & $\approx$ ($-$1.0\%) & $\approx$ (+1.0\%/0\%) \\
Whitening (multi-view) & $\uparrow$ (+2.3\%) & $\approx$ ($-$0.3\%/+0.7\%) \\
Whitening (single-view) & $\approx$ (+0.3\%) & $\approx$ (0\%/+0.2\%) \\
Multi-view (vs.\ single) & $\uparrow$ (+4.7\%) & $\uparrow$ (+0.5\%/+2.0\%) \\
\bottomrule
\end{tabular}
\end{table}

The key insight is that \textbf{cross network is essential for ranking quality}
(MRR/NDCG), even though it reduces HR by trading coverage for precision.
Whitening improves HR for multi-view (+2.3\%) by decorrelating view features.
SE-style gating has marginal effects and is not critical.
Multi-view outperforms single-view on both HR (+4.7\%) and ranking metrics.
...
This trade-off suggests different configurations for different deployment scenarios:
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Candidate generation (recall-oriented)}:
    For first-stage retrieval where maximizing relevant items in top-$K$ matters,
    consider removing explicit cross to favor broader coverage.
  \item \textbf{Final ranking (precision-oriented)}:
    For re-ranking stages where top-1 accuracy is critical,
    retain cross network and whitening to sharpen score discrimination.
  \item \textbf{Balanced deployment}:
    For systems requiring both coverage and ranking quality,
    use NDCG@$K$ as the primary metric and tune inference boost
    to balance cold-start and frequent item performance.
\end{itemize}
