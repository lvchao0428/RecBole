...
Each whitened view is projected to the backbone dimension and refined by
an SE-style channel gating (similar to SENet~\cite{hu2018senet} but
implemented as an MLP over the projected embedding without global pooling):
\begin{align}
...
\end{align}
where $\mathbf{t}_{w,i}^{(v)}$ is the whitened embedding for item $i$ in view $v$.
...
where $\mathrm{pop}(i)$ is the interaction count in training data:
\begin{equation}
...
\end{equation}
where $w_i$ only reweights the anchor (positive) samples.
...
\base + \tfidf & 3.38 & 2.42 & 2.05 & 9.83 & 6.40 & 5.35 \\
\base + \tfidf + \llm & 3.44 & 2.41 & 2.01 & 10.06 & 6.50 & 5.42 \\
\model (7B) & \textbf{3.77} & \textbf{2.60} & \textbf{2.14} & \textbf{10.55} & \textbf{6.72} & \textbf{5.55} \\
\midrule
\multicolumn{7}{l}{\textbf{Amazon Toys\&Games}} \\
\base (ID-only) & 4.61 & 2.53 & 1.87 & 10.61 & 5.92 & 4.45 \\
\base + \tfidf & 4.71 & 3.22 & 2.76 & 11.93 & 7.91 & 6.67 \\
\base + \tfidf + \llm & 4.81 & 3.28 & 2.80 & 11.97 & 7.94 & 6.69 \\
