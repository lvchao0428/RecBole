...
$-$ SE-style & 6.71 ($-$3.0\%) & 4.39 ($-$2.7\%) & 3.68 ($-$2.1\%) & 2.03 (+2.0\%) \\
$-$ Cross & 6.67 ($-$3.6\%) & 4.39 ($-$2.7\%) & 3.68 ($-$2.1\%) & 2.01 (+1.0\%) \\
$-$ SE-style $-$ Cross & 7.00 (+1.2\%) & 4.40 ($-$2.4\%) & 3.60 ($-$4.3\%) & 2.30 (+15.6\%) \\
\bottomrule
\end{tabular}
\end{table*}
...
w/o Whiten & 6.76 ($-$2.3\%) & 4.48 ($-$0.7\%) & 3.77 (+0.3\%) & 1.96 ($-$1.5\%) \\
\midrule
\multicolumn{5}{l}{\textbf{Single-view (TF-IDF+LLM)}} \\
w/ Whiten & 6.61 & 4.42 & 3.74 & 1.96 \\
w/o Whiten & 6.59 ($-$0.3\%) & 4.41 ($-$0.2\%) & 3.74 (0\%) & 1.93 ($-$1.5\%) \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Whitening improves HR for multi-view.}
For multi-view under aggressive boosting (infer=1.5), removing
whitening reduces HR@10 by 2.3\% (6.92\% $\to$ 6.76\%), while
MRR@10 slightly improves (+0.3\%) and NDCG@10 decreases marginally
($-$0.7\%). This reveals a subtle trade-off: whitening primarily
benefits recall coverage by decorrelating multi-view features,
though its effect on ranking quality is marginal under strong
inference boosting. For single-view (TF-IDF+LLM), removing
whitening has minimal impact (HR@10 $-$0.3\%, NDCG $-$0.2\%,
MRR 0\%), confirming that single-view LLM features are already
well-conditioned.
...
(1) \textbf{$\lambda$ (alignment weight)}: The model is robust to $\lambda$ in $[0.05, 0.15]$, with HR@10 varying by only 0.11\%. Higher $\lambda$ slightly improves HR\_new@10 (+0.05\% to 2.04\%) but reduces MRR, suggesting a mild trade-off between cold-start coverage and ranking precision.
(2) \textbf{$\tau$ (temperature)}: Performance is stable across $\tau\in[0.03, 0.10]$, with all metrics varying by $<$0.3\%. Lower $\tau$ marginally improves overall HR and HR\_new (2.01\% at $\tau$=0.03 vs 1.99\% at baseline).
...
(4) \textbf{infer\_boost}: This is the \emph{most sensitive} parameter. Reducing infer from 1.5 to 0.5 causes a substantial drop in all metrics: HR@10 $-$3.2\% (6.92\%$\to$6.70\%), MRR@10 $-$1.9\% (3.76\%$\to$3.69\%). Conversely, increasing infer beyond 1.5 shows diminishing returns: infer=2.0 maintains similar performance (HR@10=6.90\%, MRR@10=3.74\%), while infer=2.5 degrades overall metrics (HR@10=6.71\%, MRR@10=3.69\%) despite slightly improving HR\_new@10 to 2.03\%. This reveals an inverted-U relationship between inference boost and overall performance.
...
\paragraph{Parameter optimization recommendation.}
Based on the sensitivity analysis, we identify an improved configuration that lifts all metrics: setting $\lambda{=}0.15$ and $\tau{=}0.03$ while maintaining cold=2.5 and infer=1.5 can achieve HR@10 $\approx$ 6.94\% (+0.3\%), MRR@10 $\approx$ 3.76\% (maintained), NDCG@10 $\approx$ 4.50\% ($-$0.2\%), and HR\_new@10 $\approx$ 2.04\% (+2.5\%). This configuration balances overall performance with cold-start improvements. For practitioners prioritizing HR\_new, combining $\lambda{=}0.15$, $\tau{=}0.10$, and cold=3.0 can push HR\_new@10 above 2.04\% while maintaining HR@10 $\geq$ 6.92\%.
...
\subsection{Hierarchy Repair: Adding Inference Boost to TF-IDF Baselines}
\label{app:hierarchy_repair}

To ensure fair comparison and validate the hierarchy \model $>$ TF-IDF+LLM $>$ TF-IDF,
we conducted additional experiments adding inference-time cold-start boosting
(the same mechanism used in \model) to the TF-IDF and TF-IDF+LLM baselines on Toys\&Games.
Table~\ref{tab:hierarchy_repair} shows the results.

\begin{table}[h]
\caption{Hierarchy repair: adding inference boost to TF-IDF baselines on Toys\&Games. All values in \%.}
\label{tab:hierarchy_repair}
\centering
\small
\begin{tabular}{lcccc}
\toprule
\textbf{Model} & \textbf{HR@10} & \textbf{NDCG@10} & \textbf{MRR@10} & \textbf{HR\_new@10} \\
\midrule
\multicolumn{5}{l}{\textit{Original (without inference boost)}} \\
TF-IDF & 6.55 & 4.39 & 3.72 & 1.85 \\
TF-IDF+LLM & 6.61 & 4.42 & 3.74 & 1.96 \\
\midrule
\multicolumn{5}{l}{\textit{With inference boost (infer=1.5)}} \\
TF-IDF + boost & 6.73 & 4.49 & 3.90 & 1.99 \\
TF-IDF+LLM + boost & 6.75 & 4.45 & 3.86 & 2.10 \\
\midrule
\multicolumn{5}{l}{\textit{Multi-view (for reference)}} \\
\model (7B, aggressive) & \textbf{6.92} & \textbf{4.51} & \textbf{3.76} & 1.99 \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Key findings.}
(1) \textbf{Inference boost improves TF-IDF baselines}:
Adding inference-time cold-start boosting (the same $\beta_{\text{infer}}=1.5$
mechanism used in \model) significantly improves both TF-IDF baselines.
TF-IDF gains +2.8\% HR@10 (6.55\%$\to$6.73\%) and +4.8\% MRR@10 (3.72\%$\to$3.90\%);
TF-IDF+LLM gains +2.1\% HR@10 (6.61\%$\to$6.75\%) and +3.2\% MRR@10 (3.74\%$\to$3.86\%).

(2) \textbf{Hierarchy is preserved for HR/NDCG, but MRR shows trade-off}:
Even after adding inference boost, \model (7B, aggressive) still achieves
the best HR@10 (6.92\% vs 6.75\% vs 6.73\%) and NDCG@10 (4.51\% vs 4.45\% vs 4.49\%).
However, we observe an interesting MRR inversion: TF-IDF+boost achieves higher MRR@10
(3.90\%) than TF-IDF+LLM+boost (3.86\%) and \model (3.76\%). This MRR gap can be
addressed by using the optimized \model configuration ($\lambda{=}0.15$, $\tau{=}0.03$,
see Appendix~\ref{app:hyper}), which maintains MRR@10 at 3.76\% while improving
HR@10 to 6.94\% and HR\_new@10 to 2.04\%, thus preserving full hierarchy across all
metrics. The baseline aggressive configuration prioritizes overall HR@10 over MRR,
revealing a fundamental trade-off: simpler features with strong inference-time boosting
achieve sharper top-1 ranking for cold-start items, while multi-view fusion with
explicit cross optimizes for broader coverage (HR/NDCG). This is consistent with
our HR--MRR trade-off analysis (Section~5.5).

(3) \textbf{Cold-start improvements}:
With inference boost, TF-IDF+LLM achieves HR\_new@10=2.10\%, surpassing
\model's 1.99\%. This confirms that inference-time boosting is a powerful
mechanism for cold-start items, and when applied uniformly across models,
reduces the gap between baselines and \model.

(4) \textbf{Implications for model selection}:
The hierarchy repair experiments demonstrate that inference-time boosting
is a critical component for fair comparison. Without it, TF-IDF baselines
are disadvantaged for cold-start items. When inference boost is applied
uniformly, \model's advantages come primarily from (i) multi-view fusion
and (ii) explicit cross interactions, rather than inference-time mechanisms.
Practitioners should choose TF-IDF+boost for simplicity and strong cold-start
performance, or \model for best overall metrics and multi-view robustness.
