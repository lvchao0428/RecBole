...
# Two-phase strong baseline: SASRec_Align + TF-IDF (base only) on Amazon_Video_Games
...
python scripts/two_phase_train.py \
  ...
  --seed 2025 \
  --variant_features "sasrec,tfidf,videogames" \
  --watchdog_interval 20 \
  --watchdog_log "run_metrics/watchdog_tfidf_videogames.log" \
  --watchdog_cpu_gb 40 \
  --watchdog_gpu_gb 28 \
  --save
