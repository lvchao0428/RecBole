...
# Two-phase TF-IDF+LLM with Stratified Evaluation (ML-1M Dataset)
...
echo "========================================="
echo "TF-IDF+LLM with Stratified Metrics (ML-1M)"
echo "========================================="
echo "Dataset: ml-1m"
...
python scripts/two_phase_train.py \
  --model SASRec_Align \
  --dataset ml-1m \
  --config_files "sasrec_align_ml1m_qwen3_stratified.yaml" \
  --phase_a_grid \
  --align_grid "0.05" \
  --tau_grid "0.05" \
  ...
  --seed 2025 \
  --variant_features "sasrec,tfidf,llm,ml1m,stratified" \
  --watchdog_disable \
  --save
